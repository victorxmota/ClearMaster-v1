...
export const Reports: React.FC = () => {
  ...
  const exportPDF = () => {
    ...
    doc.text('DOWNEY CLEANING SERVICES - ACTIVITY REPORT', 14, 15);
    
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text(`Generated for: ${selectedUserFilter === 'all' ? 'All Staff' : activeUser?.name}`, 14, 22);
    ...
    autoTable(doc, {
      head: [['Date', 'Location', 'Shift Time', 'Safety Items Checked', 'Status']],
      body: tableData,
      startY: 35,
      styles: { fontSize: 8 },
  ...
  if (isLoading) return <div className="flex justify-center p-12"><Loader2 className="animate-spin text-brand-600" size={40} /></div>;
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500">Service logs and safety verification history</p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-gray-500 font-bold uppercase tracking-wider text-[10px]">
              <tr>
                ...
                <th className="p-4">Safety Checklist</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredRecords.sort((a,b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime()).map((record) => (
                <tr key={record.id} className="hover:bg-brand-50/20 transition-colors">
                  <td className="p-4 font-mono text-gray-600">{format(parseISO(record.date), 'dd/MM/yyyy')}</td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-2 text-brand-600">
                      <ShieldCheck size={14} />
                      <span className="text-[10px] font-medium" title={getSafetySummaryText(record.safetyChecklist)}>
                        {getSafetyItemsCount(record.safetyChecklist)} safety items verified
                      </span>
                    </div>
                  </td>
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded-md text-[10px] font-bold uppercase ${record.endTime ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700 animate-pulse'}`}>
                      {record.endTime ? 'Complete' : 'In Progress'}
                    </span>
                  </td>
                </tr>
              ))}
              {filteredRecords.length === 0 && (
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-400 italic">No records found for this selection.</td>
                </tr>
              )}
            </tbody>
          </table>
